...
CREATE
OR
REPLACE TYPE TYPE_STR IS TABLE OF VARCHAR2(250);
/
...
/*==============================================================*/
CREATE OR
REPLACE TYPE EN_CONCAT_IM AUTHid CURRENT_USER AS OBJECT
(
...
CREATE OR
REPLACE TYPE BODY EN_CONCAT_IM
IS
  STATIC FUNCTION ODCIAGGREGATEINITIALIZE(SCTX IN OUT EN_CONCAT_IM)
    RETURN NUMBER
  IS
BEGIN SCTX := en_concat_im(NULL);
...
/*==============================================================*/
CREATE OR
REPLACE FUNCTION en_concat(P1 VARCHAR2)
  RETURN VARCHAR2 AGGREGATE USING EN_CONCAT_IM;
/

CREATE OR
REPLACE FUNCTION split(p_str VARCHAR2, p_delimiter VARCHAR2 DEFAULT ',')
  RETURN TYPE_STR
IS
...
rs
    (rs.count)
    := substr(l_str, 1, l_delimiter_index - 1);
