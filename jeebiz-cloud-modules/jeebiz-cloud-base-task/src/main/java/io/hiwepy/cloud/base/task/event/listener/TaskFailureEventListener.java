...
import io.hiwepy.cloud.base.task.event.TaskFailureEvent;
...
@Component
public class TaskFailureEventListener implements ApplicationListener<TaskFailureEvent> {
    ...
    @Override
    public void onApplicationEvent(TaskFailureEvent event) {
        // 1、从事件对象获取任务状态信息
        TaskStatusDTO statusDTO = event.getBind();
        statusDTO.setStatus(TaskStatusEnum.FAILURE.getStatus());
        // 2、更新任务状态信息缓存
        ...
        // 4、更新任务状态
        TaskInfoEntity entity = new TaskInfoEntity().setId(statusDTO.getId()).setStatus(TaskStatusEnum.FAILURE.getStatus());
