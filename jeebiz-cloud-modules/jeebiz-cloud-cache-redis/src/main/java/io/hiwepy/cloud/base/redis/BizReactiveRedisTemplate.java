...
public class BizReactiveRedisTemplate {
    ...
    public <K> Mono<Map<Object, Object>> batchGetUserFields(K uid, Collection<Object> hashKeys) {
    ...
    public <K> Mono<Map<Object, Object>> batchGetUserFields(K uid, String... hashKeys) {
        String userKey = BizRedisKey.USER_INFO.getKey(Objects.toString(uid));
        return reactiveRedisOperation.hmMultiGet(userKey, Stream.of(hashKeys).collect(Collectors.toList()));
    }

    public <K> Flux<Map<Object, Object>> batchGetUserFields(Collection<K> uids, String... hashKeys) {
        List<String> uKeys = uids.stream().map(uid -> {
            return BizRedisKey.USER_INFO.getKey(Objects.toString(uid));
        }).collect(Collectors.toList());
    ...
    public <K> Flux<Map<Object, Object>> batchGetUserFields(Collection<K> uids, Collection<Object> hashKeys) {
        List<String> uKeys = uids.stream().map(uid -> {
            return BizRedisKey.USER_INFO.getKey(Objects.toString(uid));
        }).collect(Collectors.toList());
    ...
    public <K> Mono<Map<Object, Map<Object, Object>>> batchGetUserFields(Collection<K> uids, String identityField,
