...
@ApiModel(value = "DataPermissionNewDTO", description = "数据权限新增参数DTO")
@Getter
@Setter
@ToString
public class DataPermissionNewDTO {
     ...
     * 数据权限项集合
     */
    @ApiModelProperty(name = "items", dataType = "java.util.List<DataPermissionItemNewDTO>", value = "数据权限项集合")
    @NotNull(message = "数据权限项不能为空")
    private List<DataPermissionItemNewDTO> items = Lists.newArrayList();
    /**
     * 数据权限可用状态:（0:不可用|1：可用）
     */
