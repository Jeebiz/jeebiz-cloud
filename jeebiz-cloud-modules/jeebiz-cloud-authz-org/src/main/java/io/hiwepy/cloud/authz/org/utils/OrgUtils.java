 ...
 * All Rights Reserved.
 */
package io.hiwepy.cloud.authz.org.utils;

import com.google.common.collect.Lists;
import io.hiwepy.cloud.authz.org.dao.entities.AuthzOrganizationEntity;
import io.hiwepy.cloud.authz.org.web.dto.AuthzOrganizationTreeDTO;
...
public class OrgUtils {
    ...
    protected static List<AuthzOrganizationTreeDTO> getSubOrgList(AuthzOrganizationEntity parentNav, List<AuthzOrganizationEntity> orgList) {

        List<AuthzOrganizationTreeDTO> orgs = Lists.newArrayList();
        //筛选当前父功能模块节点的子功能模块节点数据
        List<AuthzOrganizationEntity> childOrgList = orgList.stream()
                .filter(org -> StringUtils.equals(parentNav.getId(), org.getParent()))
                .collect(Collectors.toList());
        if (!CollectionUtils.isEmpty(childOrgList)) {
            for (AuthzOrganizationEntity org : childOrgList) {
     ...
     * @return
     */
    public static List<AuthzOrganizationTreeDTO> getOrgTreeList(List<AuthzOrganizationEntity> orgList) {
        if (CollectionUtils.isEmpty(orgList)) {
            return new ArrayList<AuthzOrganizationTreeDTO>();
        }
        // 优先获得最顶层的组织机构集合
        List<AuthzOrganizationEntity> topList = orgList.stream()
                .filter(org -> StringUtils.equalsIgnoreCase(org.getParent(), "0"))
                .collect(Collectors.toList());
        List<AuthzOrganizationTreeDTO> orgs = Lists.newArrayList();
        if (!CollectionUtils.isEmpty(topList)) {

            for (AuthzOrganizationEntity org : topList) {
